...
const [loadedRecipesOrigin,setLoadedRecipesOrigin]=useState([]);
...
//     setIsLoading(false);
//     setFilteredResults(Object.entries(recipes));
//         });
...
//     const filteredData =  loadedRecipes.filter((item) => {
//             // console.log(item[1]);
//             const itemName=item[1].strMeal;
// // console.log(itemName.toLowerCase().includes(searchInput.toLowerCase()));
//             // return itemName.toLowerCase().includes(searchInput.toLowerCase());
//             itemName.toLowerCase().includes(searchInput.toLowerCase())? res.push(item):''
//         })
//         console.log(res);
//   setFilteredResults(filteredData)
// }
...
//   if (searchInput !== '') {
//   // console.log(searchValue);
//   const filteredData =  loadedRecipesOrigin.filter((item) => {
//     // console.log(Object.values(item));
//     return item.title.toLowerCase().includes(searchInput.toLowerCase())
// })
// setFilteredResults(filteredData)
...
setLoadedRecipes(Object.entries(recipes));
  ...
  for (const key in data){
    ...
    recipes.push(recipe);
        ...
        // so it wont display the recipe before data get fetched
  setIsLoading(false);
  setLoadedRecipesOrigin(recipes);
      });
...
<div role="status" className="max-w-sm animate-pulse">
  ...
  <div className={styles.bg_header}>
    ...
    <div className="relative ml-6 w-full lg:max-w-sm">
      ...
      <RecipeList recipe={searchInput !=''? filteredResults: loadedRecipes}/>
    </section>
  )
}
